...
(`zip-full-core`) that decodes every common entry codec with **no C-FFI dependencies**,
...
them un-droppable downstream. `zip-full-core` replaces the dependency with pure-Rust
decoders so the C libraries leave the build entirely.

## What you get

- **`zip-full-core`** — `ZipArchive` over any `Read + Seek`: EOCD + Zip64 + central
