...
`zstd-sys`, `lzma-sys`). The migration runs **after** `zip-core` / `zip-forensic`
...
`zip-core` is **read/decompress-only by design** and will not gain a writer.
Therefore the wave is a **split migration**, not a wholesale replacement.

## Compatibility audit (read surface)

Every read-side zip-rs API the fleet uses is covered by `zip-core`:

| zip-rs API | zip-core equivalent | Status |
|------------|---------------------|--------|
...
headers without setting up decryption/decompression. `zip-core` covers this need
two ways: `structural_view()` already exposes per-entry header fields without
decoding, and a thin `by_index_raw`-style accessor can be added to `zip-core` when
...
1. Repoint all **read** consumers from `zip` to `zip-core`.
...
`issen` workspace (read consumers → `zip-core`; write consumers → slim zip-rs) and
