...
`zip-forensic-core`'s runtime dependency tree contains only pure-Rust crates. The three
...
a fixture produced by an **independent** encoder, decoded by zip-forensic-core's pure-Rust
...
`zip-forensic-core` decrypts encrypted entries via `by_index_decrypt`/`by_name_decrypt`
...
| AES / ZipCrypto | a real encrypted-malware sample | out of scope (zip-forensic-core does not decrypt) |
...
- **Deflate (real, multi-GB):** the DFIR-Madness "Stolen Szechuan Sauce"
  ...
  entry (CRC `ff0ce1a7`). zip-forensic-core's native decode is compared **byte-for-byte to
...
- **Deflate64 (real, 4 GiB):** the SecurityNik "TOTAL RECALL" memory-forensics CTF
  ...
  CRC `de173b7f`). zip-forensic-core decodes the entries and the recorded CRC-32 (the CTF
